...
import { byStart, fmtDate, isPast, stillListed, tRange } from "../lib/date";
import { findGrp, findUser, grpN, isAdmin, rosterOf } from "../lib/people";
import { isPublic, tally } from "../lib/rsvp";
import { A, BO, BP, C, M } from "../theme";
import { Av } from "../ui/Avatar";
import { ActImg, Cover } from "../ui/Cover";
import { BkI, CalI, PinI, UsrI } from "../ui/icons";

export function DetPage({ go, onSelectAct, events, grp, onRsvp, rsvps, openUser, mem, toggleJoin }) {
  const g = findGrp(grp);
  const all = events.filter(e => e.g === g.name && isPublic(e));
  const grpEvts = all.filter(stillListed).sort(byStart);
  const grpPast = all.filter(isPast).sort((a, b) => byStart(b, a)).slice(0, 3);
  const grpActs = acts.filter(a => a.g === g.name).sort((a, b) => (b.d || "").localeCompare(a.d || "")).slice(0, 3);
  return (
    <div style={{ paddingBottom: 20 }}>
      ...
      <div style={{ position: "relative", height: 160 }}>
        ...
        <div style={{ position: "absolute", bottom: 0, padding: 16 }}>
          ...
          <div style={{ color: "rgba(255,255,255,0.8)", fontSize: 12 }}>{grpN(g, mem)}人 • {g.since}設立</div>
        </div>
      </div>
      <div style={{ padding: "14px 16px", display: "flex", flexDirection: "column", gap: 14 }}>
        <div style={C}>
          <div style={{ padding: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
              ...
              <span style={{ fontSize: 12, color: "#999" }}>{grpN(g, mem)}人</span>
            </div>
            <div style={{ display: "flex", gap: 14, overflowX: "auto", scrollbarWidth: "none" }}>
              {rosterOf(mem, g.name).map(id => findUser(id)).map(m => (
            ...
            <button onClick={() => toggleJoin(g.name)}
              style={{ ...(rosterOf(mem, g.name).includes(ME) ? BO : BP), marginTop: 12 }}>
              {rosterOf(mem, g.name).includes(ME) ? "✓ 参加中" : "グループに参加"}
            </button>
          </div>
        </div>
        ...
        <div style={{ ...C, padding: 14 }}>
          ...
          {grpEvts.map((e, i) => {
            const done = isPast(e);
            return (
              <div key={i} style={{ border: "1px solid #eee", borderRadius: 8, padding: 12, marginBottom: 10, opacity: done ? 0.72 : 1 }}>
                <div style={{ fontWeight: 600, fontSize: 14, color: "#111" }}>
                  {done && <span style={{ fontSize: 10, fontWeight: 700, color: "#aaa", background: "#f0f0f0", borderRadius: 4, padding: "1px 6px", marginRight: 6 }}>終了</span>}
                  {e.t}
                </div>
                <div style={{ display: "flex", gap: 10, marginTop: 6 }}><span style={M}><CalI />{fmtDate(e.start)} {tRange(e)}</span><span style={M}><PinI />{e.l}</span></div>
                <EvtNote e={e} />
                {!done && <EvtActions e={e} onRsvp={onRsvp} rsvps={rsvps} />}
              </div>
            );
          })}
        </div>
        <div style={{ ...C, padding: 14 }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: "#111", marginBottom: 10 }}>活動履歴</div>
          {grpPast.length === 0 && <div style={{ color: "#bbb", fontSize: 12, padding: "4px 0" }}>まだ活動履歴はありません</div>}
          {grpPast.map(e => (
            <div key={e.id} style={{ display: "flex", gap: 10, padding: "9px 0", borderBottom: "1px solid #f4f3f0" }}>
              <span style={{ width: 3, borderRadius: 2, background: g.color, flexShrink: 0 }} />
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#111" }}>{e.t}</div>
                <div style={{ ...M, marginTop: 4 }}><CalI s={12} />{fmtDate(e.start)} {tRange(e)}</div>
              </div>
              <span style={{ ...M, alignSelf: "center" }}><UsrI s={12} />{tally(rsvps, e).yes.length}</span>
            </div>
          ))}
        </div>
        <div style={{ ...C, padding: 14 }}>
